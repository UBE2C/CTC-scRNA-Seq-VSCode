...
packages <- c("tidyverse", "BiocManager", "Seurat", "biomaRt", "AnnotationDbi",
              "scales", "EnsDb.Hsapiens.v86", "RCurl", "cowplot", "rebus",
...
#Define the script version for output files, for easier version control, and make a dedidcated folder in the WD
define_version_create_folder = function() {
    #define the current script version
    #NOTE: the version numbers must march the defined pattern.
    script_version <- str_extract(rstudioapi::getActiveDocumentContext()$path, "V" %R% one_or_more(ASCII_ALNUM) %R% one_or_more(PUNCT) %R%
...
#set the new wd to be the script version specific folder for ease of use (and ease of vesion control)
...
#note:the function automatically creates the GeneCountTable object in global environmen, no assignment needed
...
write_csv(GeneCountTable, paste0(script_version, "/", "Unstranded_GeneCountTable_re-seq", "_", script_version, ".csv"))


#From this point on we can work with the clean gene count table
GeneCountTable <- read.csv(file = paste0(script_version, "/", "Unstranded_GeneCountTable_re-seq", "_", script_version, ".csv"))
head(GeneCountTable)
...
missingNames <- gene_names[stri_isempty(gene_names$external_gene_name) == TRUE, 1]
...
clean_gene_names <- gene_names[stri_isempty(gene_names$external_gene_name) == FALSE, ]
...
##Data trimming (only if necessary)

#Following some deliberation I decided to remove patients who have only 0 time-point samples and the ones who did not get actinium
#Based on the selection I created a new, trimmed sample set
#trimmed_set <- read.table("Trial_grouping.txt", header = TRUE)
#trimmed_set <- arrange(trimmed_set, Code)
#tCode <- trimmed_set$Code
#rm(trimmed_set)


#based on the trimmed sample set, I will leave only the corresponding samples in the oGCT
#the new trimmed GeneCountÍTable will be named tGCT
#tGCT <- data.frame()
for (e in seq_along(tCode)) {
    
    split_param <- str_detect(colnames(oGCT), tCode[e])
    tmp <- oGCT[split_param]
...
read_metadata = function(path = getwd(), filename, sep) {
    ...
    print("The metadta is loaded as the following object: meta")
}
#read_metadata(filename = "Corrected_metadata_for_seurat.csv", sep = ";")
#meta$PlatePos <- NULL
...
#I'll also prepare a trimmed metadata for a trimmed seurat object
#trimmedMeta <- data.frame()
for (e in seq_along(tCode)) {
    
    split_param <- str_detect(rownames(meta), tCode[e])
    tmp <- meta[split_param, ]
    
    if(e == 1) {
        trimmedMeta <- tmp 
    } else {
        trimmedMeta <- rbind(trimmedMeta, tmp)
    }
    rm(split_param, tmp)
}


#following trimming, remove the original count table and metadata to free up some memory
#rm(GeneCountTable, oGCT, meta)


#create a trimmed seurat object for the downstream analysis
#tCTC_reseq.obj <- CreateSeuratObject(counts = tGCT,
#                                    project = "CTC_reseq",
#                                    assay = "RNA_seq",
#                                    meta.data = trimmedMeta,
#                                    min.cells = 0)
#glimpse(tCTC_reseq.obj@meta.data)
#save(tCTC_reseq.obj, file = "tCTC_RNA_re-seq_Seur_obj.RData")
...
save(CTC_reseq.obj, file = paste0(script_version, "/", "CTC_RNA_re-seq_full_seur_obj", "_", script_version, ".RData"))
...
ggsave(filename = paste0("Cell numbers", "_", script_version, ".png"), plot = cn_p,
...
ggsave(filename = paste0("Mithocondrial gene expression", "_", script_version, ".png"), plot = mt_pd,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
rm(mt_pd)
...
ggsave(filename = paste0("mRNA count distribution", "_", script_version, ".png"), plot = nCount_p,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
rm(nCount_p)
...
ggsave(filename = paste0("Gene count distribution", "_", script_version, ".png"), plot = nFeature_p,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
rm(nFeature_p)
...
ggsave(filename = paste0("Cell complexity distribution", "_", script_version, ".png"), plot = nFpernC_p,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
rm(nFpernC_p)
...
ggsave(filename = paste0("nCount vs nFeature - QC plot", "_", script_version, ".png"), plot = nCvnF_p,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
rm(nCvnF_p)
...
ggsave(filename = paste0("Filtered cell numbers", "_", script_version, ".png"), plot = cn_p,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
rm(cn_p)
...
ggsave(filename = paste0("Filtered mithocondrial gene expression", "_", script_version, ".png"), plot = mt_pd,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
rm(mt_pd)
...
ggsave(filename = paste0("Filtered mRNA count distribution", "_", script_version, ".png"), plot = nCount_p,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
rm(nCount_p)
...
ggsave(filename = paste0("Filtered gene count distribution", "_", script_version, ".png"), plot = nFeature_p,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
rm(nFeature_p)
...
ggsave(filename = paste0("Filtered cell complexity distribution", "_", script_version, ".png"), plot = nFpernC_p,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
rm(nFpernC_p)
...
ggsave(filename = paste0("Filtered nCount vs nFeature - QC plot", "_", script_version, ".png"), plot = nCvnF_p,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
rm(nCvnF_p)
...
#Plotting the result using the cell cycle phase overlay
...
ggsave(filename = paste0("Cell cylce phase clustering (split)", "_", script_version, ".png"), CellCyc_split_p2,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
...
ggsave(filename = paste0("Cell cylce phase clustering", "_", script_version, ".png"), CellCyc_p2,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
...
sct_CTC.obj <- RunPCA(sct_CTC.obj, assay = "SCT", approx = FALSE, verbose = FALSE) #approx = FALSE to run with normal vsd method
...
ggsave(filename = paste0("Principal component weights", "_", script_version, ".png"), PC_weight_p2,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
...
ggsave(filename = paste0("PCA_clustering_gb", "_", script_version, ".png"), PCA_p2,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
rm(PCA_p, PCA_p2)
...
ggsave(filename = paste0("UMAP_clustering_gb", "_", script_version, ".png"), UMAP_p2,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
rm(UMAP_p, UMAP_p2)
...
ggsave(filename = paste0("TSNE_clustering_gb", "_", script_version, ".png"), TSNE_p2,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
rm(TSNE_p, TSNE_p2)

#removing unnecessary objects
rm(g2m.IDs, s.IDs, MT_genes)
...
ggsave(filename = paste0("UMAP_nFeature_gb", "_", script_version, ".png"), UMAP_nF_p2,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
...
ggsave(filename = paste0("UMAP_nCount_gb", "_", script_version, ".png"), UMAP_nC_p2,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
...
ggsave(filename = paste0("UMAP_mt.percent_gb", "_", script_version, ".png"), UMAP_mt.percent2,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
...
ggsave(filename = paste0("UMAP_Cell_cycle_gb", "_", script_version, ".png"), UMAP_CellCyc2,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
...
ggsave(filename = paste0("UMAP_Treatment_cycles_gb", "_", script_version, ".png"), UMAP_TreatCyc2,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
...
ggsave(filename = paste0("UMAP_Treatment_type_gb", "_", script_version, ".png"), UMAP_TreatType2,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
...
ggsave(filename = paste0("UMAP_Response_group_gb", "_", script_version, ".png"), UMAP_RespG2,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
...
ggsave(filename = paste0("UMAP_Lu-177_treatment_doses_gb", "_", script_version, ".png"), UMAP_Lu2,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
rm(UMAP_Lu, UMAP_Lu2)
...
ggsave(filename = paste0("UMAP_Ac-225_treatment_doses_gb", "_", script_version, ".png"), UMAP_Ac2,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
rm(UMAP_Ac, UMAP_Ac2)
...
ggsave(filename = paste0("UMAP_Marker_Status_gb", "_", script_version, ".png"), UMAP_MarkerStat2,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
...
ggsave(filename = paste0("Treatment cycles in clusters", "_", script_version, ".png"), treatment_cyc_bar_p,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
...
ggsave(filename = paste0("Treatment types over clusters", "_", script_version, ".png"), treatment_type_bar_p,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
...
ggsave(filename = paste0("Cell cycle over clusters", "_", script_version, ".png"), cell_cyc_bar_p,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
rm(cell_cyc_bar_p)
...
#Plotting the treatment response distribution
...
ggsave(filename = paste0("Treatment response over clusters", "_", script_version, ".png"), response_group_bar_p,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
...
ggsave(filename = paste0("Surface marker over clusters", "_", script_version, ".png"), surface_marker_bar_p,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 1500, height = 1000, units = "px")
...
write_csv(x = DE_Cluster_0v1_geneNames, file = paste0(script_version, "/", "DE_clusters_0v1_bg", "_", script_version, ".csv"))
...
ggsave(filename = paste0("DEGs_Volcano_plot_gb", "_", script_version, ".png"), volcano_p,
       device = "png", path = paste0(script_version, "/", "Plots/"),
...
save_image(p = volcano, file = paste0(script_version, "/", "Plots/", "DE_volcano_plot", "_", script_version, ".png"), width = 1500, height = 1000, scale = 1)
...
Surf_mark_df <- data.frame(EpCAM = sct_CTC.obj[["SCT"]]@data[c("ENSG00000119888"), ],
                           ...
                           Cluster = sct_CTC.obj$seurat_clusters, RespGroup = sct_CTC.obj$RespGroup,
...
ggsave(filename = paste0("EpCAM_expression_over_cycles_Responders", "_", script_version, ".png"), EpCAM_expr_over_cyc_resp_p,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 3500, height = 3000, units = "px")
...
ggsave(filename = paste0("EpCAM_expression_over_cycles_Nonresponders", "_", script_version, ".png"), EpCAM_expr_over_cyc_nresp_p,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 3500, height = 3000, units = "px")
...
ggsave(filename = paste0("PSMA_expression_over_cycles_Responders", "_", script_version, ".png"), PSMA_expr_over_cyc_resp_p,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 3500, height = 3000, units = "px")
...
ggsave(filename = paste0("PSMA_expression_over_cycles_Nonresponders", "_", script_version, ".png"), PSMA_expr_over_cyc_nresp_p,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 3500, height = 3000, units = "px")
...
export_Seurat_for_scVelo = function(seurat_object, gene_IDs_names_df, ENSEMBL_ID_col, gene_names_col) {
    ...
    write.csv(scVelo_sct_CTC.obj@meta.data, file = paste0(script_version, "/", "Velocity data", "/", "CTC_metadata_for_scVelo", "_", script_version, ".csv"),
    ...
    writeMM(gene_count_matrix_IDs, file = paste0(script_version, "/", "Velocity data", "/", "CTC_GeneCountMatrix_IDs_for_scVelo", "_", script_version, ".mtx"))
    ...
    writeMM(gene_count_matrix_names, file = paste0(script_version, "/", "Velocity data", "/", "CTC_GeneCountMatrix_gNames_for_scVelo", "_", script_version, ".mtx"))
    ...
    write.csv(seurat_object@reductions$pca@cell.embeddings, file = paste0(script_version, "/", "Velocity data", "/", "PCA_data_for_scVelo", "_", script_version, ".csv"),
              quote = FALSE, row.names = FALSE)
    ...
    write.table(data.frame("Gene" = rownames(gene_count_matrix_IDs)), file = paste0(script_version, "/", "Velocity data", "/", "ENSEMBL_IDs_for_scVelo", "_", script_version, ".csv"),
    ...
    write.table(data.frame("Gene" = gene_names_to_save), file = paste0(script_version, "/", "Velocity data", "/", "Gene_names_for_scVelo", "_", script_version, ".csv"),
                quote = FALSE, row.names = FALSE, col.names = FALSE)
    ...
    message("\n",
            ...
            "\n", paste0("CTC_metadata_for_scVelo", "_", script_version, ".csv"),
            "\n", paste0("CTC_GeneCountMatrix_IDs_for_scVelo", "_", script_version, ".mtx"),
            "\n", paste0("CTC_GeneCountMatrix_gNames_for_scVelo", "_", script_version, ".mtx"),
            "\n", paste0("PCA_data_for_scVelo", "_", script_version, ".csv"),
            "\n", paste0("ENSEMBL_IDs_for_scVelo", "_", script_version, ".csv"),
            "\n", paste0("Gene_names_for_scVelo", "_", script_version, ".csv")
    )
...
GSEA_DE_logP_ranked_df <- GSEA_DE_lst_geneNames[, c(6, 8)]
...
read_gmt = function(path) {
    ...
    for (e in seq_along(file_names)) {
        ...
        #loading a gmt file into a temporary object
        tmp_gmt <- read.gmt(paste0(gmt_files, "/", file_names[e]))
...
run_GSEA = function(random_seed = 1234,
    ...
    write.csv(GSEA_result_df, file = paste0(script_version, "/", save_folder, "/", GSEA_csv_file_name, "_", script_version, ".csv"))
    ...
    for (e in 1:nrow(GSEA_result@result)) {
        ...
        ggsave(filename = paste0(plot_file_name, "-", GSEA_result@result[e,1], "_", script_version, ".png"), gsea_res_p,
...
run_multi_GSEA = function(reference_lst,
    ...
    pathway_names <- str_extract(reference_lst, one_or_more(ASCII_ALNUM) %R% "." %R% one_or_more(ASCII_ALNUM) %R% "." %R% one_or_more(ASCII_ALNUM))
...
write.csv(hallmark_gsea_res_df, file = paste0(script_version, "/", "GSEA_results/", "hallmark_set_GSEA", "_", script_version, ".csv"))
...
for (e in 1:nrow(hallmark_gsea_res@result)) {
    ...
    print(h_gsea_res_p)
    ggsave(filename = paste0("GSEA hallmark set", "-", hallmark_gsea_res@result[e,1], "_", script_version, ".png"), h_gsea_res_p,
           device = "png", path = paste0(script_version, "/", "GSEA_results", "/", "Plots", "/"),
           width = 3500, height = 3000, units = "px")
...
ggsave(filename = paste0("NetGSA_filtered_DEGs_Volcano_plot_gb", "_", script_version, ".png"), volcano_p,
       device = "png", path = paste0(script_version, "/", "Plots/"),
       width = 3500, height = 3000, units = "px")
rm(volcano_p, upreg, downreg, topupreg, topdownreg)
