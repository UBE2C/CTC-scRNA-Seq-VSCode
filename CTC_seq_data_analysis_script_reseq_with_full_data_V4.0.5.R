...
packages <- c("tidyverse", "stringi", "BiocManager", "Seurat", "glmGamPoi", "biomaRt", "AnnotationDbi", "scales", "EnsDb.Hsapiens.v86",
                "RCurl", "cowplot", "rebus", "ggsci", "EnhancedVolcano", "progress", "metap", "doSNOW", "foreach", "scCustomize", "graphite",
                    "org.Hs.eg.db", "fgsea", "clusterProfiler", "Matrix", "ggpubr", "SeuratWrappers", "presto", "multtest")
...
if (file.exists("Re-seq_sample_extended_metadata.csv")) {
    message("The processed metadata file is already present in the working directory.",
    "\n", "No further processing will be done.") 
} else if (file.exists("Re-seq_sample_extended_fixed_metadata_with_markers.csv")) {
...
read_metadata(filename = "Re-seq_sample_extended_fixed_metadata_with_markers.csv", sep = ";")
glimpse(meta)
head(meta)
...
# Save the so far loaded and processed data as a .RData object
...
if (exists("sct_CTC.obj", where = .GlobalEnv)) {
    ...
    filt_CTC.obj <- readRDS(file = paste0(getwd(), "/", script_version, "/", "SCTransformed_CTC_Seurat_object", script_version, ".rds"))
...
check_multimapped_IDs = function(multi_ID_dataframe) {
    ...
    for(element in multi_mapped$ensembl_gene_id) {
        ...
        duplicated_genes[element] <-unique(entrezIDs$external_gene_name[grep(element, entrezIDs$ensembl_gene_id)])
    }
...
# Attaching ENTREZ IDs to the DE genes using the following functions
# This is the original, well trusted for loop based implementation
...
entrezID_matching_par(DE_Cluster_0v1, unifiedEntrezIDs, "ensembl_gene_id", "entrezgene_id")
...
# ENTREZ ID matching function
entrezID_matching_par(DE_Cluster_0v1_geneNames, unifiedEntrezIDs, "ensembl_gene_id", "entrezgene_id")
...
# Save the resulting DEG list
write_csv(x = DE_Cluster_0v1_geneNames_entrezIDs, file = paste0(script_version, "/", "DEG_clusters_0v1", script_version, ".csv"))




## As for further analysis, I will be mainly using the ENTREZ IDs, I will remove genes from the DEG list which have no ENTREZ ID


# Filter the original list into a new, clean variable
DEG_cluster_0v1_clean <- dplyr::filter(DE_Cluster_0v1_geneNames_entrezIDs, entrezIDs != "NA")


# Save the new, clean DEG list for future use
...
if (!exists(x = "DEG_cluster_0v1_clean", where = .GlobalEnv)) {
...
entrezID_matching_par(GSEA_DE_lst_geneNames, unifiedEntrezIDs, "ensembl_gene_id", "entrezgene_id")
...
# This will be done based on the ENTREZ IDs
...
# Remove the NAs from the GSEA DEG list
...
# Assign the clean GSEA DEG list to a new variable
GSEA_DEG_list_clean <- GSEA_DE_lst_geneNames_entrezIDs


# Save the clean GSEA DEG list for further use
write_csv(x = GSEA_DEG_list_clean, file = paste0(script_version, "/", "GSEA_DEG_list_clean", script_version, ".csv"))


# Remove unnecessary variables
rm(GSEA_DE_lst, GSEA_DE_lst_geneNames, GSEA_DE_lst_geneNames_entrezIDs)
...
if (!exists("GSEA_DEG_list_clean", where = .GlobalEnv)) {
    GSEA_DEG_list_clean <- read.csv(file = paste0(script_version, "/", "GSEA_DEG_list_clean", script_version, ".csv"))
    message("The clean GSEA DEG list was loaded as the following object: ", deparse(substitute(GSEA_DEG_list_clean)))
}
...
GSEA_DEG_ranked_df <- GSEA_DEG_list_clean[, c(6, 7, 8)]
GSEA_DEG_ranked_df_ord <- dplyr::arrange(GSEA_DEG_ranked_df, desc(Log_and_p_Ranking))
head(GSEA_DEG_ranked_df_ord)


# Plot cluster 0 top 50 upregulated
ggplot(GSEA_DEG_ranked_df_ord[1:50, ],
...
# Plot cluster 0 top 50 downregulated
ggplot(GSEA_DEG_ranked_df_ord[nrow(GSEA_DEG_ranked_df_ord):(nrow(GSEA_DEG_ranked_df_ord) - 50), ],
       aes(x = entrezIDs, y = Log_and_p_Ranking)) +
    geom_point() +
    theme_classic() +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
run_multi_GSEA(reference_lst = pathways,
               DE_gene_list_multi = GSEA_DEG_list_ord,
...
a <- Seurat::RidgePlot(sct_CTC.obj,
            features = c("ENSG00000175063",
                        "ENSG00000134057",
                        "ENSG00000131747",
                        "ENSG00000166851",
